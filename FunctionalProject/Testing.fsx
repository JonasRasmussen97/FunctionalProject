namespace Project
    ...
    module Testing =
        ...
        // Generates the random id to be used.
        let fileId = fileIdGenerator()
        ...
        // First command here.Test for getting a file and matching the id up against the model & the api.
            let getFileMetaInformation = { new Command<API, string>() with
                        // Here we retrieve the id of the API file.
                override __.RunActual api = api.fileMetaInformationById(0, fileIdGenerator()); api
                        // Here we retrieve the id of the Model file.
                override __.RunModel fileName = (Model.files.Head.Item (fileIdGenerator())).name
                        // Here we match the two id's to see if the states of both the api and the model are equal.
                override __.Post(api, fileName ) = api.fileMetaInformationById(0, fileIdGenerator()).name = fileName  |@ sprintf "model: %s <> api: %s" fileName (api.fileMetaInformationById(0,fileIdGenerator()).name)
        ...
        // Check directory meta information.
            ...
            { new ICommandGenerator<API,string> with
                ...
                member __.Next model = Gen.elements [getFileMetaInformation; getDirectoryMetaInformation]}
