...
module Model =
   ...
   // type OrbitDirectory = {id: int; name: string; path: string; version: int; parent_id: API.DirectoryParent; is_checked_out: bool; is_default: bool}
    // Defines a file response from the service part of the API.
    //type ServiceFile = {id: int; name: string; size: string; mimetype: string; parent_id: API.DirectoryParent; version: int; created_at: string; modified_at: string; ms_timestamp: string; path: string; snapshots_enabled: bool}
    // Defines the structure of a user.
   // type User = {id: int; name: string; initials: string}
    
    [<StructuredFormatDisplay("{currentFileId}")>]
    type InModel = {
        FileMetaData: API.FileMetaData list
        directories : API.DirectoryMetaData list
        currentFileId: int
        // OrbitDirectory : OrbitDirectory list
        // ServiceFile: ServiceFile
         
        
        }
 (*
    let s = {id=2; version=1; name="README.txt"; parentId=15; timestamp="637479675580000000"}
